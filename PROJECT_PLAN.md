...
- [ ] Deck (палуба) - если можно извлечь
- [ ] Cabin_Count (количество кают у пассажира)

### 3.6 Взаимодействия признаков
- [ ] Sex_Pclass (комбинация пола и класса)
- [ ] Age_Pclass (возраст и класс)
- [ ] Fare_per_Person (Fare / Family_Size)
- [ ] Title_Age (титул и возраст)

### 3.7 Нормализация и масштабирование
- [ ] Логарифмирование Fare (если распределение перекошено)
- [ ] Стандартизация числовых признаков (для некоторых моделей)
...
- [ ] One-Hot Encoding для номинальных признаков (Embarked, Id_cabin)
- [ ] Label Encoding для порядковых признаков (Sex, Title)
- [ ] Проверка корректности кодирования

### 4.2 Разделение данных
- [ ] Разделение train на train/validation (80/20 или 70/30)
- [ ] Использование StratifiedSplit для сохранения баланса классов
- [ ] Альтернатива: использование cross-validation

### 4.3 Финальная проверка
- [ ] Проверка отсутствия пропусков
- [ ] Проверка типов данных
- [ ] Сохранение списка финальных признаков
- [ ] Подготовка test.csv аналогичным образом
...
- [ ] Дополнительный feature engineering на основе ошибок
...
- [ ] Обучение финальной модели на всем train.csv
- [ ] Предсказания для test.csv
- [ ] Проверка формата предсказаний (0 или 1)

### 7.2 Подготовка submission файла
- [ ] Создание DataFrame с PassengerId и Survived
- [ ] Проверка количества строк (418 + заголовок)
- [ ] Проверка отсутствия NaN значений
- [ ] Сохранение в CSV формат

### 7.3 Валидация submission
- [ ] Проверка формата файла
- [ ] Проверка диапазона значений (0 или 1)
- [ ] Проверка количества записей

---

## Этап 8: Итерации и улучшения

### 8.1 Анализ результатов
- [ ] Сравнение с baseline (gender_submission ~76%)
- [ ] Анализ ошибок на Kaggle leaderboard
- [ ] Изучение public notebooks для идей

### 8.2 Дополнительные улучшения
- [ ] Дополнительный feature engineering
- [ ] Попробовать другие комбинации признаков
- [ ] Эксперименты с различными моделями
- [ ] Анализ важности признаков

### 8.3 Документация
- [ ] Комментарии в коде
- [ ] Описание выбранных признаков
...
1. **Завершить EDA** - полностью понять данные
2. **Обработать пропуски** - критично для качества модели
3. **Feature Engineering** - ключевой этап для улучшения accuracy
4. **Базовые модели** - установить baseline
5. **Продвинутые модели** - Random Forest, XGBoost
6. **Ансамбли** - комбинирование лучших моделей
7. **Submission** - первая отправка на Kaggle
8. **Итерации** - улучшение на основе результатов

---

## Целевые метрики

- **Baseline:** ~76% (простое правило: все женщины выжили)
- **Хороший результат:** 80-85%
- **Отличный результат:** 85-90%
- **Топ результат:** 90%+
...
- [Titanic Discussion Forum](https://www.kaggle.com/competitions/titanic/discussion)
- [Public Notebooks](https://www.kaggle.com/competitions/titanic/code)
- Документация: pandas, scikit-learn, XGBoost, LightGBM

---

## Заметки

- Все изменения в данных должны применяться одинаково к train и test
- Важно избежать data leakage (утечки данных)
- Использовать cross-validation для надежной оценки
- Документировать все эксперименты и их результаты
